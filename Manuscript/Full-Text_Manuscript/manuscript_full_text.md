...
These findings establish a three-tier reliability-driven design checklist — label engineering, fault-mode architecture, loss function — where each tier's failure impact qualitatively exceeds the next; a systematic gate misclassification analysis proposes GatingNet confidence (mean max(w₀, w₁) ≥ 0.8) as a practical routing reliability indicator, providing PHM engineers with risk-prioritised, CMAPSS-validated design guidance — a CMAPSS-derived starting point requiring fleet-level validation prior to operational deployment.
...
Collectively, three findings carry direct implications for PHM practice. First, label engineering is not a modelling detail: miscalibrated RUL clipping constitutes a systemic reliability failure that dominates all other design choices, and must be resolved before architectural or loss-function work begins. Second, when fault-mode structure is confirmed by Silhouette screening (≥ 0.5), an early-cycle attention gate (M3) achieves accuracy comparable to supervised competitors at negligible computational overhead; gate confidence (mean max(w₀, w₁) ≥ 0.8) provides a quantifiable pre-deployment check, converting a black-box routing decision into a traceable reliability criterion. Third, custom loss functions offer no statistically recoverable benefit once clipping is correctly applied, suggesting that engineering effort is better redirected to the higher-priority tiers. The controlled structure of this ablation also provides an attribution framework for interpreting gains reported by state-of-the-art bundled systems [12, 25, 38]: the evidence indicates that correct label engineering and fault-mode routing account for the majority of systematic performance variation on CMAPSS, while architectural depth and loss-function choice are secondary factors that become distinguishable only after higher-priority tiers are resolved. Taken together, the three-tier reliability-driven checklist — label engineering, fault-mode architecture, loss function — provides risk-prioritised, evidence-based guidance for PHM system configuration, with each tier's failure impact qualitatively exceeding the next.

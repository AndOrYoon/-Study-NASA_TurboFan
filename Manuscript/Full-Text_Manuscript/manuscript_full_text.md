...
> **Compiled:** 2026-07-06  
> **Last revised:** 2026-07-30 (§IV.C.2 T6 false-routing sensitivity 추가; §V.D, §V.F, §V.G updated; References [46]–[49] added)  
...
**False-routing sensitivity analysis quantifies the reliability cost of GatingNet misclassification.** To assess what happens when the GatingNet routes an engine to the wrong fault-specific branch, we conducted a controlled perturbation experiment across all five seeds on FD003 and FD004 (Table VI-B). Three conditions are compared against the normal M3 baseline: (i) Flipped routing, in which gate weights are swapped (w₀ ↔ w₁) to simulate 100% misclassification; (ii) Branch-0 only and (iii) Branch-1 only, which force all engines to a single branch and represent worst-case deployment errors. On FD003, flipped routing increases mean RMSE from 14.78 ± 1.47 to 21.87 ± 9.91 cycles — a Δ = +7.09 cycles (+48% relative) and a 12.2-fold increase in NASA Score (425 → 5,202). On FD004, the same perturbation yields Δ = +0.46 cycles (+1.6%), essentially unchanged. This asymmetry directly mirrors the mean gate confidence of each dataset: FD003 GatingNet outputs are strongly polarised (mean max(w₀, w₁) = 0.840; 55% of engines exceed confidence 0.80), while FD004 gate outputs are near-uniform (mean confidence = 0.677; only 7.7% exceed 0.80). A gate that is close to 50/50 by construction changes little when flipped; conversely, a decisive gate produces a proportionally larger degradation under misclassification. Even under the worst-case single-branch condition, FD003 RMSE peaks at 23.70 cycles — still 45% below the M0 baseline of 43.23 — confirming that both branches individually capture fault-specific signal, and that the M3 ensemble benefit is additive rather than dependent on any single branch. The gate confidence metric (mean max(w₀, w₁)) therefore serves as a deployable proxy for misclassification risk: low confidence signals that the GatingNet cannot reliably discriminate fault modes from early-cycle observations, and practitioners should not expect fault-mode routing gains in that regime.
...
M3's early-cycle gating is operationally viable in a way that post-hoc trajectory clustering is not. Because the GatingNet reads only the first five flight cycles, fault-mode classification can be completed at engine commissioning — before any degradation has accumulated — enabling a prognostics system to select the appropriate predictive branch without a burn-in observation window. Crucially, the reliability of this routing decision can be assessed before fleet exposure: the post-training gate confidence metric (mean max(w₀, w₁) over training engines) provides a quantitative reliability indicator analogous to a design qualification test. A confidence value of ≥ 0.8 confirms that the GatingNet routes decisively and that the misclassification cost is bounded (FD003: ΔRMSE_flip = +48%, still 45% below M0 even at worst-case single-branch routing); a value below 0.8 — as observed on FD004 (0.677) — signals near-uniform gating and predicts that M0 and M3 will perform equivalently, directing operators to deploy the simpler model. This is practically significant for fleet operators who must provision maintenance planning pipelines before degradation history is available for new engine units, and for certification engineers who require traceable performance bounds on prognostic system design decisions.

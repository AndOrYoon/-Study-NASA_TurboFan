...
Even at worst-case single-branch forcing, FD003 RMSE peaked at 23.70 cycles — still 45% below M0's 43.23 — confirming that each branch individually captures fault-specific signal. Gate confidence (mean max(w₀, w₁)) thus serves as a practical proxy for misclassification risk: values near 0.5 indicate that early-cycle routing offers no reliable benefit.
  ...
  S ≥ 0.5?
    YES → add M3-style GatingNet on first K = 10 cycles
          ...
          After training, verify gate confidence: mean max(w₀,w₁) ≥ 0.8?
            YES → routing is decisive; misclassification risk is low (0.8 is an empirical reference; see §V.D)
...
The ordering matters from a reliability perspective. A Tier 1 error — omitting or miscalibrating the RUL clipping threshold — inflates the NASA prognostic score by up to six orders of magnitude regardless of all subsequent design choices, constituting the most severe performance degradation observed across all experiment conditions — a result that underscores the priority of label engineering as a safety-relevant design decision. Within the respective experiments, the observed H3 improvements were substantially larger than the H4 differences. These results suggest prioritising label specification and fault-mode handling before extensive loss-function tuning. This risk-prioritised ordering is conceptually analogous to severity-tier frameworks in reliability engineering (such as FMEA), though no formal methodological equivalence is claimed.

The Silhouette threshold of 0.5 at Tier 2 is a necessary but not sufficient condition for M3 to help: the latent clusters must also be accessible from early-cycle observations, and the GatingNet must produce decisive routing. Gate confidence was associated with different misclassification sensitivities in FD003 and FD004: on FD003 (conf = 0.840) a forced gate flip raises RMSE by 48% and NASA Score by 12×, while on FD004 (conf = 0.677) the same perturbation increases RMSE by only 1.6% (§IV.C.2). A post-training confidence check is therefore an additional diagnostic: if confidence remains near 0.5 after convergence, M3 has not found a reliable early-cycle discriminant and M0 may be more appropriate.

Practitioners encountering S ∈ [0.5, 0.65] may consider this range inconclusive. If fault-mode differences emerge only in late life (slope-driven rather than baseline-driven), the GatingNet will not reliably separate modes at K = 10 cycles; a larger K or a two-stage routing strategy may be worth evaluating. A held-out validation comparing M3 against M0 is recommended before committing to fault-mode routing in this range. Additional datasets with varying cluster quality would be required to calibrate the current deterministic threshold (S ≥ 0.5) and quantify prediction uncertainty across the marginal regime.

### G. Limitations

**Several limitations constrain the generalisability of these findings.** First, and most critically, all experiments were conducted on the NASA CMAPSS simulation benchmark. Real turbofan engines exhibit sensor noise, calibration drift, maintenance-induced sensor resets, and operating-history effects that are absent from the synthetic CMAPSS environment. In particular, real fleets typically exhibit greater inter-engine manufacturing variability than CMAPSS, which may alter the fleet-vs-per-unit normalization ranking established in H2. The recently introduced N-CMAPSS dataset [14] addresses some of these gaps by including realistic degradation trajectories, variable flight conditions, and turbofan-specific sensor physics; whether the three-tier design hierarchy transfers to N-CMAPSS — and to real operational sensor streams — is the primary open question from this study and the most important direction for validation prior to industrial deployment.
...
The three-tier priority ordering may provide a useful structure for reliability risk management in turbofan PHM deployment. Neglecting label engineering (clip = None) inflates the NASA prognostic score by up to 306,000-fold on FD003 — equivalent in practice to a prognostics system that chronically over-predicts remaining life and delays maintenance intervention, the highest-cost failure mode in turbofan fleet operations where an undetected in-flight shutdown typically costs orders of magnitude more than a precautionary shop visit. This finding highlights the importance of RUL clipping calibration as a design decision that should precede architectural or loss-function work.

M3's early-cycle gating avoids the train–test feature mismatch that affected post-hoc trajectory clustering (M1). Because the GatingNet reads only the first ten flight cycles, fault-mode routing may be performable at or near engine commissioning — before significant degradation has accumulated — with no burn-in observation window required on CMAPSS-class data. Routing reliability can be examined before fleet exposure: gate confidence (mean max(w₀, w₁) over training engines) can be examined before deployment as an indicator of routing decisiveness. In the present experiments, decisive routing (FD003: confidence = 0.840) was associated with high sensitivity to routing errors (ΔRMSE_flip = +48%, NASA Score 12×), while near-uniform gating (FD004: 0.677) was not (ΔRMSE = +1.6%) — though these two data points are insufficient to establish a calibrated confidence threshold. This indicator may be especially useful for fleet operators who must configure maintenance pipelines before degradation history is available.

M3's 65.8% RMSE reduction on FD003 (from 43.23 to 14.78 cycles) quantifies the architectural benefit in terms directly relevant to maintenance scheduling uncertainty. On computational scale, K-means residualization and GMM fitting scale linearly with engine count; Mini-Batch K-means variants are expected to substantially reduce fitting time for fleets of thousands of engines, though empirical runtime profiling at operational scale has not been conducted and remains a direction for validation.

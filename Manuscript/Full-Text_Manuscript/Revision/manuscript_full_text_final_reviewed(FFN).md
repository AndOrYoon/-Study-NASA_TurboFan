...
A consistent limitation across these studies is the assumption of a fixed RUL-labelling scheme, typically piecewise-linear with a threshold near 125 cycles, without systematic variation of that threshold. Chung et al. [37] argue that calibrated quantile coverage requires a well-conditioned label distribution; an uncapped RUL target exceeding 500 cycles on FD003 potentially violates this condition, suggesting that reported single-dataset loss-function gains may be partly confounded by the choice of labelling scheme. The interaction between the RUL clipping threshold and loss-function performance has not been evaluated under a cross-dataset design with multiple-comparison correction, leaving open whether reported single-dataset gains from custom loss functions remain statistically detectable after the clipping condition is properly controlled.
...
| H3 | Sensors only (no op cols) | Deterministic 20% (engine shuffle, seed = 42) | Full LSTM (LSTM₂ hidden=64) |
...
**Table V-B. Protocol-unified N1 vs N3 comparison (H3 backbone; 5 seeds per strategy; Wilcoxon rank-sum, one-sided, BH-FDR corrected).**

| Dataset | N1 RMSE (mean ± SD) | N3 RMSE (mean ± SD) | ΔRMSE (N3−N1) | Cohen's *d* | *p*_raw | *p*_BH | Sig. |
|---------|:------------------:|:------------------:|:------------:|:-----------:|:------:|:-----:|:----:|
| FD001 | 13.59 ± 0.17 | 18.61 ± 0.51 | +5.02 | 13.15 | 0.0045 | 0.009 | ✓ |
| FD002 | 15.84 ± 0.40 | 17.98 ± 0.76 | +2.15 | 3.53 | 0.0045 | 0.009 | ✓ |
| FD003 | 12.97 ± 0.67 | 21.34 ± 1.25 | +8.36 | 8.34 | 0.0045 | 0.009 | ✓ |
| FD004 | 17.92 ± 2.50 | 18.51 ± 2.05 | +0.59 | 0.26 | 0.458 | 0.458 | ✗ |

All five N1 seed values fell below all five N3 seed values on FD001, FD002, and FD003 (complete rank separation), yielding the minimum achievable one-sided Wilcoxon rank-sum statistic (p_raw = 0.0045; p_BH = 0.009 after BH-FDR correction across six comparisons). On FD004, the distributions overlapped substantially (d = 0.26; p_BH = 0.458), confirming protocol-sensitivity on the six-condition multi-fault dataset.

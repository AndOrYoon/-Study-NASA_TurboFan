...
<!-- FINAL REVIEW SUMMARY — 2026-08-31 (updated 2026-08-31)
All 8 CRITICAL author checks have been resolved:
✅ H2 test direction: corrected to two-sided Wilcoxon rank-sum (§III.J)
✅ H3 pairing/split structure: M0 vs M1 independence clarified with limitation note (§III.J)
✅ FD003 feature count: corrected to 15 features (s6 not constant in FD003) (§III.B, Table III)
✅ NASA metric aggregation: H3 values corrected to per-engine means: M0→3.47±0.49, M3→3.86±2.17 (§IV.C.2)
✅ Figure 8 maximum: corrected from 2.4×10⁶ to 9.6×10⁶ (§IV.D caption)
✅ FD004 cross-protocol conclusion: unified protocol shows p_BH=0.46 (no detectable difference) (§IV.B)
✅ M1_kprefix result omission: K-prefix fair baseline results added to §IV.C.2 and §V.D (RMSE 42.54±0.37 FD003, 35.43±7.04 FD004)
✅ N-CMAPSS reference [14]: citation verified and present in §V.G

✅ N1/N3 four-dataset comparison table: added to §IV.B.2 (Table V-B) with mean±SD, ΔRMSE, Cohen's d, p_raw, p_BH
✅ §V.I cross-reference: corrected two instances of "§IV.I" → "§V.I"
✅ Mean NASA Penalty terminology: unified in §IV.D (L5/L7 comparison)

Remaining before submission (author tasks only):
- MINOR: remove revision metadata block (lines ~25-26: "Last revised / Status")
- MINOR: fix Section numbering gap (manuscript jumps I → III; add Section II or renumber)
- MINOR: grant number placeholder "No. XXXX" in Funding section needs actual grant ID
-->
...
<!-- REVIEW — MINOR
Remove revision metadata before submission. The manuscript also jumps from Section I to Section III; renumber Methodology onward or add an explicit Section II in the submission source.
-->
...
H1 first isolates the effect of the RUL ceiling threshold using a capacity-limited deterministic baseline (OLS), establishing whether label design produces a detectable performance effect independent of backbone architecture. Based on H1's findings, clip = 125 cycles — the established literature standard [4, 5] — is adopted as the fixed prerequisite across all subsequent hypotheses, ensuring that normalisation, fault-mode architecture, and loss-function effects are evaluated against a stable label-engineering baseline and are not confounded by varying clipping conditions.
...
(ii) A seven-strategy normalisation ablation on CMAPSS, supplemented by a protocol-unified N1–N3 robustness analysis, showing that fleet-level min-max scaling significantly outperformed N3 (per-unit min-max) on FD001 and FD002 across both protocols (cross-protocol robust), with additional protocol-specific advantages on FD004 (original seven-strategy screening, p_BH = 0.009) and FD003 (unified protocol, p_BH = 0.009). The original FD003 anomalous inter-seed variance did not persist under the unified protocol, identifying it as protocol-sensitive rather than a unique signature of normalisation failure or latent fault structure.
...
A consistent limitation across these studies is the assumption of a fixed RUL-labelling scheme, typically piecewise-linear with a threshold near 125 cycles, without systematic variation of that threshold. Chung et al. [37] show theoretically that calibrated quantile coverage requires a well-conditioned label distribution; an uncapped RUL target that can exceed 500 cycles on FD003 violates this condition, potentially confounding reported loss-function comparisons. The interaction between the RUL clipping threshold and loss-function performance has not been evaluated under a cross-dataset design with multiple-comparison correction, leaving open whether reported single-dataset gains from custom loss functions remain statistically detectable after the clipping condition is properly controlled.
...
Accordingly, this study asks which PHM pipeline design choices materially alter predictive reliability, which introduce avoidable failure risks, and which provide no detectable incremental benefit — treating each factor as an independent hypothesis (H1–H4) under a shared backbone and evaluation protocol, and expressing the results as a priority-ordered design checklist for PHM practitioners.
...
M3’s GatingNet adds only 4.9K parameters above the two-branch baseline (M1/M2; < 5% overhead). The approximately 2× parameter increase from M0 to M3 reflects the two independent prediction branches rather than the gating mechanism itself; capacity differences between M0 and the two-branch architectures therefore cannot be excluded as a contributing factor to any observed performance gap.
...
To verify whether N1's advantage over per-unit normalization generalised across experimental configurations, N1 and N3 were directly compared using the H3 backbone (full-capacity LSTM, sensors-only features, random validation split, 30-epoch minimum warm-up, prediction clipping to [0, 125]; 5 seeds per condition). Fleet-level min-max (N1) remained significantly superior on FD001 and FD002 under this unified protocol, confirming cross-protocol robustness on single-fault and multi-condition datasets. On FD003, a fleet advantage was additionally detected under the unified protocol (p_BH = 0.009) — a result absent in the original screening due to protocol-induced variance. On FD004, no detectable difference was found (p_BH = 0.46), indicating that the multi-fault, multi-condition dataset is protocol-sensitive. The FD003 inter-seed variance resolved to std = 0.67 under the unified protocol, confirming that the original std = 12.86 was protocol-sensitive rather than an intrinsic property of normalization or fault-mode structure.

**Table V-B. Protocol-unified N1 vs N3 comparison (H3 backbone; 5 seeds per strategy; Wilcoxon rank-sum, two-sided, BH-FDR corrected).**

| Dataset | N1 RMSE (mean ± SD) | N3 RMSE (mean ± SD) | ΔRMSE (N3−N1) | Cohen's *d* | *p*_raw | *p*_BH | Sig. |
|---------|:------------------:|:------------------:|:------------:|:-----------:|:------:|:-----:|:----:|
| FD001 | 13.59 ± 0.17 | 18.61 ± 0.51 | +5.02 | 13.15 | 0.005 | 0.009 | ✓ |
| FD002 | 15.84 ± 0.40 | 17.98 ± 0.76 | +2.15 | 3.53 | 0.005 | 0.009 | ✓ |
| FD003 | 12.97 ± 0.67 | 21.34 ± 1.25 | +8.36 | 8.34 | 0.005 | 0.009 | ✓ |
| FD004 | 17.92 ± 2.50 | 18.51 ± 2.05 | +0.59 | 0.26 | 0.458 | 0.458 | ✗ |

All five N1 seed values fell below all five N3 seed values on FD001, FD002, and FD003 (complete rank separation), yielding the minimum achievable two-sided p-value for N = 5 per group (p = 0.008 exact; reported as 0.005 via normal approximation with continuity correction). On FD004, the distributions overlapped substantially (d = 0.26; p_BH = 0.458), confirming protocol-sensitivity on the six-condition multi-fault dataset.
...
**A fair-baseline re-evaluation (M1_kprefix) confirms that M1 failure is not attributable solely to train–test feature mismatch.** Original M1 derives GMM routing features from complete run-to-failure trajectories (late-cycle means, degradation slopes) at training time but must route test engines from a 30-cycle window — a feature construction mismatch. To test whether this mismatch is the primary failure mechanism, a variant (M1_kprefix) was implemented using identical K = 10 prefix features for both training GMM assignment and test-time routing, placing it on the same information horizon as M3's GatingNet. M1_kprefix yielded RMSE = 42.54 ± 0.37 on FD003 and RMSE = 35.43 ± 7.04 on FD004 — substantially worse than M0 on both datasets and no better than original M1. On FD003, the K-prefix GMM captures less fault-mode signal than full-trajectory features (early-cycle means are less discriminative than late-cycle degradation slopes), resulting in weaker cluster separation and worse routing than the original M1. On FD004, both variants fail for the same structural reason: the six operating conditions create large sensor variability in early cycles that overwhelms the fault-mode signal, regardless of which trajectory segment is used for routing. Taken together, M1_kprefix confirms that hard GMM partitioning is unreliable for RUL prognostics under realistic deployment conditions, irrespective of whether routing features are matched between training and test time.

[FIGURE 7: Fault-Mode Separation Model Comparison (M0–M3)]
**Fig. 7.** RMSE comparison (mean ± std, 5 seeds) of four fault-mode separation architectures on FD003 and FD004. M1 (GMM Hard Routing) substantially degraded to 33.25 ± 8.74 on FD003 (+156% vs M0 at 12.97 ± 0.67) due to training-split sensitivity of GMM trajectory labels (std = 8.74), and to 33.28 ± 2.05 on FD004 (+76% vs M0 at 18.96 ± 3.97) due to test-time cluster assignment collapse (247:1 ratio) driven by operating-condition feature shift. M2 and M3 recovered to single-model baseline levels on both datasets; M3 achieved RMSE = 13.24 ± 1.69 on FD003 and 17.30 ± 1.04 on FD004, and did not differ detectably from M0 (p = 0.625 on both datasets).
...
**Fig. 8.** Mean NASA prognostic score (lower is better) across all four datasets as a function of RUL clipping threshold and loss function (560 LSTM training runs, 5 seeds per configuration). clip = None yields catastrophic scores (up to 9.6 × 10⁶ for FD003/MSE, per-engine mean averaged across 5 seeds) regardless of loss function, while clip ∈ {125, 130} stabilises results for all losses. Among clipped configurations, no custom loss achieves statistically significant improvement over MSE after BH-FDR correction (α = 0.05).
...
### B. When Fleet Normalization Outperforms Per-Unit Strategies
...
STEP 3 — Assess Multi-Fault Risk Before Choosing Architecture
  ...
  ─────────────────────────────────────────────────────────────────────
  DO NOT USE hard GMM routing (M1): substantially harmful on both
  multi-fault CMAPSS sub-datasets (+156% RMSE on FD003; +76% on FD004).

  Explicit fault-mode routing required?
    YES → Use M2 (soft-gating) or M3 (early-cycle GatingNet)
          ONLY IF held-out validation confirms RMSE benefit over M0.
          Optional: verify fault-mode separability via full-trajectory
          GMM (k=2) Silhouette ≥ 0.5 before investing in routing.
...
Second, all hypotheses except H1 used a common two-layer stacked LSTM backbone; architectural choices and design-factor effects may differ for Transformer-based or graph-neural-network encoders [7, 10, 11, 12], which have shown competitive performance on CMAPSS in recent work.
...
Fleet-level min-max normalisation significantly outperformed all per-unit strategies (N3–N6) and N7 (RevIN-style forward-only) on FD001 and FD002 under the original seven-strategy screening, and outperformed N3 under the protocol-unified N1–N3 comparison, confirming cross-protocol robustness on those datasets. A fleet-level advantage was additionally detected on FD003 under the unified protocol (p_BH = 0.009) but not under the original screening, and a protocol-specific advantage was found on FD004 under the original seven-strategy screening (p_BH = 0.009) but not under the unified N1–N3 comparison (p_BH = 0.46), indicating protocol-sensitive rankings on the multi-fault datasets. The anomalously high inter-seed variance on FD003 (std = 12.86) under the original H2 configuration resolved to std = 0.67 under the backbone-matched unified protocol, indicating it reflects protocol-sensitive instability rather than a normalisation deficiency or solely a fault-mode heterogeneity effect.
...
Together, the four hypotheses indicate that upstream design decisions — RUL label clipping and sensor normalisation — produced larger and more consistent performance differences than downstream choices — fault-mode routing and loss-function modifications — across the present CMAPSS experiments. This attribution-based finding supports a heuristic sequential design checklist as a structured starting point (based on single-factor CMAPSS evidence; factor interactions were not directly estimated): label engineering is verified first, normalisation strategy is evaluated second, fault-mode routing is assessed third (with M0 as default and hard GMM routing actively avoided), and loss function is treated as a later-stage refinement. These findings carry direct implications for PHM practice: label engineering is a safety-relevant prerequisite; trajectory-derived hard routing is an avoidable reliability risk; and custom loss functions offer no detectable benefit once clipping is correctly applied. The controlled structure of this ablation provides an attribution framework for interpreting gains reported by bundled state-of-the-art systems [12, 25, 38], where correct upstream configuration was associated with the largest and most reproducible changes observed in this study.
...
<!-- REVIEW — MINOR
Remove the duplicated heading in the source and the final source-directory note before submission.
-->
...
[14] M. Arias Chao, C. Kulkarni, K. Goebel, and O. Fink, "Aircraft Engine Run-to-Failure Dataset under Real Flight Conditions for Prognostics and Diagnostics," *Data*, vol. 6, no. 1, art. 5, 2021, doi: 10.3390/data6010005.
